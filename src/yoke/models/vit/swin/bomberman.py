...
class ScalarTemporalConditionedLodeRunner_9band(nn.Module):
    ...
    def __init__(
        ...
        if phase_fourier_bands > 0:
            # Min period 1.0 d: a 0.3 d floor (study 076) regressed RMSE and
            # inflated per-band biases -- the sub-day frequencies aliased/overfit
            # against the 2 d cutoff rather than resolving the early decline.
            # Max stays 200 d (full curve length). phase_freqs is a persistent
            # buffer, so old checkpoints restore their own trained frequencies on
            # strict load -- this bound only affects fresh studies.
