...
| 111 | _RUNNING_ | _TBD_ | **MEDIAN-WEIGHTED PINBALL** — 106 champion with the SOLE change QUANTILE_WEIGHTS=(1,3,1)→normalized (0.2,0.6,0.2) in the pinball combine (was equal .mean() = (⅓,⅓,⅓)). Decoder scope, waist 8, N_QUANTILES=3, delta/anchor/rollout off, batch 5, EMA off (0.99 no-op) | **Reallocates loss capacity to the point forecast.** Pinball trains 3 quantiles independently; the equal-weight combine spent ⅔ of the gradient positioning the q10/q90 band we don't score, only ⅓ on the median (index 1) that drives RMSE (L2/mean). Weights normalized to sum 1 so loss scale / effective LR unchanged — only relative emphasis shifts (verified: uniform weights reproduce the old .mean() to allclose). Median now 0.6 of the combine vs 0.2 each outer. Hypothesis: sharper point forecast, esp. the skewed blue-band tail where median≠mean. **Risk: too much median weight → outer quantiles decalibrate and stop regularizing (098 showed a bare point head reintroduced blue bias); (1,3,1) is a moderate first step, (1,8,1) would be the aggressive follow-up.** @1000 arbiter; Δ ≲0.03 = inside ±0.02 seed floor, needs re-run to confirm. Implementation: PinballLoss gained optional quantile_weights arg (loderunner.py), default None = byte-identical legacy behavior. |
