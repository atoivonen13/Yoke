...
| 109 | _RUNNING_ | _TBD_ | **60 EPOCHS** — 108 config (backbone mult 0.03, decoder scope) with total_epochs 40→60 AND terminal_steps scaled ×1.5 so the SAME cosine schedule is stretched over 60 epochs (NOT 40-epoch decay + 20 flat/rising). Tests whether the ~ep15 loss-curve plateau is a real floor or the run just needs longer at the champion config. Clean epochs-only A/B vs 108. **⚠️ terminal_steps MUST scale ×1.5 in the launch CSV — with num_cycles=0.5 the cosine RISES back toward anchor_lr past terminal_steps, so leaving it at the 40-epoch value makes ep41–60 run at a climbing LR, not annealed.** @1000 arbiter; also compare ep40 vs ep60 checkpoints directly. |
