...
| 110 | _RUNNING_ | _TBD_ | **EMA (Polyak) decay 0.1→0.99 + eval --use_ema** — 106 champion (decoder scope, mult 0.1, 40 epochs) with the SOLE change ema_decay 0.1→0.99. **EMA was effectively a no-op until now:** decay 0.1 = ~1.1-step window (shadow ≈ instantaneous weights) AND eval defaults to --use_ema=False, so no champion number ever contained any averaging. 0.99 = ~100-step (~1.2-epoch) window, matching the argparse default the config had drifted from. Shadow covers all requires_grad params (conditioner + output_head + unfrozen decoder); frozen encoder correctly excluded. **MUST eval with --use_ema to see the effect** (the shadow is saved either way, so it's ONE training run, two evals: raw vs --use_ema). Targets the jagged per-batch trajectory in the loss curve. Expected small (±0.01–0.02); residual is the info-limited blue-band tail, EMA polishes optimization noise. @1000 arbiter. |
