...
| 085 | _this commit_ | _TBD_ | **backbone ON**, waist 8, **tail unfrozen** (scope="tail", mult=0.1), quantile head, **BATCH_SIZE=2** (eff. 8) | **Batch-2 twin of 082** — the clean control for 084. 082 (tail) and 084 (decoder) differ only in scope but ran at different batch. This reruns tail scope at batch 2 so 085-vs-084 is a batch-matched tail-vs-decoder A/B: isolates "does unfreezing the whole decoder beat just the tail?" from the batch/LR regime. Tail retains ~no backbone activations → batch 2 fits easily. |
